...
(async function main() {
    ...
    //const MODEL_URL = urlParams.get('model') || 'altrum.glb';
    const MODEL_URL = (new URL(window.location.href)).searchParams.get('model') || 'altrum.glb';

    // Загрузка модели
    const { json, binBuffer } = await loadGLB(MODEL_URL);
