...
function computeSceneAABB(meshes, binBuffer) {
  ...
  meshes.forEach(meshData => {
    // Вычисляем локальный AABB для этого меша (по его позициям)
    ...
    meshData.mesh.primitives.forEach(prim => {
      ...
      for (let i = 0; i < posData.length; i += 3) {
        for (let j = 0; j < 3; j++) {
          if (posData[i+j] < localMin[j]) localMin[j] = posData[i+j];
          if (posData[i+j] > localMax[j]) localMax[j] = posData[i+j];
        }
      }
    });
    ...
    // Трансформируем каждый угол матрицей модели и обновляем глобальные границы
    for (const corner of corners) {
      mat4.transformPoint(tempPoint, meshData.modelMatrix, corner);
      for (let j = 0; j < 3; j++) {
        if (tempPoint[j] < globalMin[j]) globalMin[j] = tempPoint[j];
        if (tempPoint[j] > globalMax[j]) globalMax[j] = tempPoint[j];
      }
    }
  });
